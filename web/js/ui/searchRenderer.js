...
function _renderBrowseInfoModal(ui, hit) {
    ...
    const _nodeExists = () => _workflowInfoNodeId !== null && !!(app.graph && app.graph.getNodeById(_workflowInfoNodeId));

    const sendToWorkflowBtn = document.createElement('button');
    sendToWorkflowBtn.className = 'civitai-button small';
    sendToWorkflowBtn.style.cssText = 'background:var(--cfy-accent,#5c8aff);color:#fff;';
    sendToWorkflowBtn.title = 'Add or update model info as a Note node in the current workflow';
    sendToWorkflowBtn.innerHTML = _nodeExists()
    ...
    sendToWorkflowBtn.addEventListener('click', () => {
        ...
        parts.push(`Type: ${modelTypeApi}`);
        parts.push(`Creator: ${creator}`);
        if (uniqueBaseModels.length > 0) parts.push(`Base Model: ${uniqueBaseModels.join(', ')}`);
        parts.push(`Model ID: ${modelId}`);
        parts.push(`URL: https://civitai.com/models/${modelId}${primaryVersionId ? '?modelVersionId=' + primaryVersionId : ''}`);
        if (publishedAt) parts.push(`Published: ${publishedAt}`);
        if (allVersions.length > 0) parts.push(`Versions: ${allVersions.length}`);
        if (allTrainedWords.length > 0) {
            parts.push('');
            parts.push('Trigger Words:');
            parts.push(allTrainedWords.join(', '));
        }
        if (tags.length > 0) {
            parts.push('');
        ...
        try {
            ...
            if (!LG || !app.graph) {
                ui.showToast('ComfyUI graph not available.', 'error');
                return;
            }

            let wfNode = _nodeExists() ? app.graph.getNodeById(_workflowInfoNodeId) : null;

            if (wfNode) {
                // Update existing node
                wfNode.properties = wfNode.properties || {};
                wfNode.properties.text = noteText;
                if (wfNode.widgets && wfNode.widgets[0]) {
                    wfNode.widgets[0].value = noteText;
                }
                app.graph.setDirtyCanvas(true, true);
                ui.showToast('Workflow note updated!', 'success', 2000);
            } else {
                // Create new Note node
                wfNode = LG.createNode('Note');
                if (!wfNode) {
                    ui.showToast('Could not create Note node.', 'error');
                    return;
                }
                ...
                let px = 100, py = 100;
                if (cvs && cvs.ds) {
                    px = -cvs.ds.offset[0] + cvs.canvas.width  / 2 / cvs.ds.scale - 200;
                    py = -cvs.ds.offset[1] + cvs.canvas.height / 2 / cvs.ds.scale - 150;
                }
                wfNode.pos = [px, py];
                wfNode.size = [400, 300];
                wfNode.properties = wfNode.properties || {};
                ...
                _workflowInfoNodeId = wfNode.id;
                app.graph.setDirtyCanvas(true, true);
                ui.showToast('Model info added to workflow!', 'success', 2000);
            }

            sendToWorkflowBtn.innerHTML = '<i class="fas fa-sync-alt"></i> Update Workflow';
        } catch (err) {
    ...
    footerRight.append(copyIdBtn, sendToWorkflowBtn, viewBtn);
