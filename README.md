...
- 各カードに最新バージョンのファイルサイズを表示。Info パネルでは全バージョンのサイズを一覧で確認できます
- NSFW サムネイルは設定したしきい値を超えるとぼかし表示(クリックで表示)。Settings で **Unlock everything** を選ぶと、ぼかしなし・除外なしで全モデルが見えます
...
| **NSFW ぼかししきい値** | この `nsfwLevel` 以上のサムネイルをぼかす(0–128)。**Unlock everything** を選ぶとぼかしが外れるだけでなく、Civitai 側の結果除外も停止します — 画像がすべてエクスプリシットなモデルはこの設定でのみ一覧に出ます |
...
- **モデルカードにダウンロードサイズを表示。** Civitai の検索インデックスにはファイル情報が一切含まれないため、ページごとに1回のバッチリクエストで REST API から解決し(`server/routes/ModelFileSizes.py`)、カードが表示された後に埋め込みます — 閲覧が待たされることはありません。公開済みバージョンのサイズは変わらないので、プロセスの生存期間中キャッシュします。Info パネルでは全バージョンのサイズを一覧表示します。バージョンが20個ある時に実際に選ぶ基準はこれです。
- **「Unlock everything」がぼかしだけでなく結果の除外も解除するようになりました。** 検索リクエストには常にレーティングフィルターがかかっており、モデルの `nsfwLevel` は*配列*であるため、画像がすべてエクスプリシットなモデルは丸ごと除外されていました — LoRA だけで 11,504 件が、何も表示されないまま見えない状態でした。Unlock 設定では Browse と Search が全レーティングを要求します。
- **修正: 初回起動時にモデルタイプの選択欄が空白になる問題。** デフォルト値が `checkpoint` でしたが、選択肢は ComfyUI のフォルダ名(`checkpoints`)由来です。存在しない値を `<select>` に代入すると静かに空白になり、Download フォームにモデルタイプが無い状態になっていました。保存値を実在する選択肢に対して解決するようにしました。
- **修正: 長いバージョン名が Browse カードからはみ出す問題。** `text-overflow` は他の子要素に挟まれた素のテキストノードには効かないため、省略されずに最大 61px カードの外へ飛び出していました。
- **修正: タッチ端末で操作オーバーレイが全サムネイルを覆う問題。** ホバーが無いためオーバーレイが開きっぱなしになり、モデル画像が1枚も見えませんでした。画像をタップするとそのカードだけ操作が開くようになりました。
- **修正: Browse カードの「All (N)」ボタンが何も起きない問題。** 旧 Search タブのマークアップにしか存在しないコンテナを開閉しようとしていました。全バージョンが並ぶ Info パネルを開くようにしました。
- **修正: Start download の上に空の枠だけの箱が表示される問題。** `:empty` で隠す指定はありましたが、中に HTML コメントがあるため `:empty` が一度も一致していませんでした。
- **その他の修正:** 折り返したヒント文中の `<code>` チップの重なり、細いカードでのタイプ/ベースモデルバッジの衝突、Activity でファイル名が `Meg…` まで潰れる問題、折り返したフォーム項目間の余白の二重取り。
- **スマートフォン・タブレット・デスクトップ幅で UI 全体を見直し。** 行が折り返した時にツールバーのラベルが別のコントロールの横に取り残される問題を解消(幅がインラインスタイルに書かれており、あらゆるメディアクエリに勝っていました)。Directories はパスとリセットボタンが同じ行に並びます。タッチ環境では全体を約40pxのタップ領域に — 掴める高さが4pxしかなかったカードサイズのスライダーを含みます。7タブ × 5つの幅 × 2テーマで全要素を走査し、はみ出し・枠外脱出・重なり・小さすぎるタップ領域を検出するスクリプトで検証し、すべて0件を確認しました。
- **死んでいた Search タブを削除。** Browse に置き換わって以降、レンダラー・ハンドラー・スタイルシートの該当セクションは到達不能でした — 約350行。
